...
public interface ApDao {
    ...
    String INSERT_FIELDS = "username, Alng, Alat, Blng, Blat, Clng, Clat, created_time";
    String SELECT_FIELDS = "id, " + INSERT_FIELDS;

    //插入
    @Insert({"insert into ", TABLE_NAME,"(",INSERT_FIELDS,
            ") values (#{username},#{Alng},#{Alat},#{Blng},#{Blat},#{Clng},#{Clat},#{created_time})"})
    int addApLocation(Location location);
    ...
    //更新定位信息
    @Update({"update", TABLE_NAME, "set Alat=#{Alat}, Alng=#{Alng} where username=#{username}"})
